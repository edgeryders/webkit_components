  ...
  <div class="timeline"  id="events_container">
    ...
    <div class="filters md:filters-md mb-4 w-full" v-if="filtered()">
      ...
      <div class="filter type" v-if="type" @click="clear('type')">
        <p class="key" :style="{background: type.color}">type</p><p>{{type.name}}</p>
      </div>
      <div class="filter type" v-if="selectedDate" @click="clear('selectedDate')">
        <p class="key" :style="uiStyle('highlight', custom.style)">date</p><p>{{selectedDate | formatDate}}</p>
      </div>
      <div class="filter location" v-if="location" @click="clear(location)">
        <p class="key" :style="uiStyle('highlight', custom.style)">location</p><p>{{location}}</p>
      </div>
      <div class="filter search" v-if="search" @click="search = ''">
        <p class="key" :style="uiStyle('highlight', custom.style)">search</p><p>{{search}}</p>
      </div>
    </div>

    <transition-group name="list" tag="div" class="events_list">
    <div v-for="(item, index) in filteredItems" :key='index' class="day md:day-md mb-4" :class="{active: isActive(item.event.start) }" :style="textStyle('paragraph', custom.style)">
